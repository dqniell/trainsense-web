...
# NOTE: GTFSService (pandas/numpy) is imported lazily inside the data-load
# endpoints below so that read/realtime endpoints don't pull in heavy deps.
# This keeps serverless (Vercel) bundles small and cold starts fast.
...
def _build_stop_count_lookup():
    ...
    precomputed_path = os.path.join(gtfs_dir, 'route_stop_seq.json')

    # Serverless / trimmed deploys don't ship the large stop_times.txt. When it's
    # absent, load the precomputed route_id -> {stop_id -> sequence} map instead
    # (generated by scripts/build_route_stop_seq.py). Locally, with stop_times.txt
    # present, we always rebuild from source so the map stays fresh.
    if not os.path.exists(stop_times_path) and os.path.exists(precomputed_path):
